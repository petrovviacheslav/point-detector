...
import api from "../../api";
import {authorize, setToken} from "../../store/reducers";
...
function AuthForm() {
    ...
    const emailHandlerLogin = (e: string) => {
        setEmail(e)
        if (e.length < 1) {
            setEmailError('Некорректный email')
        } else {
    ...
    const authHandler = (e: { preventDefault: () => void; }, type: string) => {
        e.preventDefault();
        if (type === "login") {
            api.loginReq(email, password)
                .then(data => {

                    (document.getElementById("two_factor") as HTMLElement).classList.remove("hidden-code");
                    (document.getElementById("overlay-modal") as HTMLElement).classList.remove("hidden-code");
    ...
    const close = () => {
        (document.getElementById("two_factor") as HTMLElement).classList.add("hidden-code");
        (document.getElementById("overlay-modal") as HTMLElement).classList.add("hidden-code");
        setSeconds(0);
    }
        ...
        <div className={"second-root"}>
            ...
            <div className="hidden-code overlay" id="overlay-modal">
                <div id={"two_factor"} className={"hidden-code two-factor"}>
                    ...
                    <input className={"two-factor-input"} onChange={e => {
                    ...
                    <button disabled={!(seconds === 0)} onClick={event => authHandler(event, 'login')}>
