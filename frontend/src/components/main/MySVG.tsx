...
import api from "../../api";
import {addPoint, updatePercent} from "../../store/reducers";
...
const MySVG = () => {
    ...
    const rFromStore: number = useSelector((state: RootState) => state.token.r)
    const tokenFromStore: string = useSelector((state: RootState) => state.token.token)

    const clickSVG = (event: React.MouseEvent<SVGSVGElement>) => {
        event.preventDefault()
        //const svg = document.getElementById('paint') as SVGSVGElement;
        ...
        let cursorpt = pt.matrixTransform((svg.getScreenCTM() as DOMMatrix).inverse());

        let reallyX = cursorpt.x - 150;
        let reallyY = 150 - cursorpt.y;

        api.sendPoint({
            x: Number((reallyX / 25).toFixed(2)),
            y: Number((reallyY / 25).toFixed(2)),
            r: rFromStore
        }, tokenFromStore)
            .then(p => {
                dispatch(addPoint(p))
    ...
    return (
        <svg onClick={clickSVG} height="300" width="300" xmlns="http://www.w3.org/2000/svg" id="paint">
