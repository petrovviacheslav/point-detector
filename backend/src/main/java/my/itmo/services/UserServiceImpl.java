...
public class UserServiceImpl implements UserService {
    ...
    @Autowired
    private EmailConfirmationTokenGeneratorService confirmationTokenGenerator;
...
//    private final PointService pointService;
    ...
    public UserDTO getCurrentUser() {
        return userRepository.findById(getUserIdFromToken())
                ...
                .map(user -> new UserDTO(user.getId(), user.getEmail(), user.getUsername(), user.getIsVerified())).get();
    ...
    public AuthResponse register(RegisterRequest regRequest) {
        ...
        try {
            if (checkUser.isEmpty()) {
                ...
                String verifiedCache = confirmationTokenGenerator.generateToken();

                log.info("cache = {}", verifiedCache);
                ...
                emailService.sendConfirmationEmail(user.getEmail(), "Регистрация", "Здравствуйте! Спасибо за регистрацию. Подтвердите вашу почту, перейдя по ссылке http://parzi.ru/point-detector/verification?cache=" + verifiedCache);
    ...
    @Override
    public ResponseEntity checkCacheUser(ConfirmProfileRequest req) {
        User user = userRepository.findById(getUserIdFromToken())
                .orElseThrow(() -> new UsernameNotFoundException("Нет юзера с id: " + getUserIdFromToken()));

        try {
            if (Objects.equals(req.cache(), user.getVerifiedCache())) {
                user.setVerifiedCache(null);
